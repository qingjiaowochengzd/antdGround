...
const Index = ({ membermarket, dispatch, loading, location }) => {
  const { list, pagination } = membermarket
  const { query = {}, pathname } = location

  const listProps = {
    pagination,
    dataSource: list,
    loading: loading.effects['membermarket/query'],
    onChange (page) {
...
Index.propTypes = {
  membermarket: PropTypes.object,
  loading: PropTypes.object,
  location: PropTypes.object,
  dispatch: PropTypes.func,
}

export default connect(({ membermarket, loading }) => ({ membermarket, loading }))(Index)
