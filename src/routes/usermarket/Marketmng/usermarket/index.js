...
const Index = ({ usermarket, dispatch, loading, location }) => {
  const { list, pagination } = usermarket
  const { query = {}, pathname } = location

  const listProps = {
    pagination,
    dataSource: list,
    loading: loading.effects['usermarket/query'],
    onChange (page) {
...
Index.propTypes = {
  usermarket: PropTypes.object,
  loading: PropTypes.object,
  location: PropTypes.object,
  dispatch: PropTypes.func,
}

export default connect(({ usermarket, loading }) => ({ usermarket, loading }))(Index)
